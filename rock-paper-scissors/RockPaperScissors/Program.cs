...
namespace RockPaperScissors
{
    class Program
    {
        static Dictionary<string, IPlayer> AIPlayers = new Dictionary<string, IPlayer>()
        {
            ...
            { "StubbornAI", new StubbornAI(1) },
            { "ShortAttentionSpanAI", new ShortAttentionSpanAI() }
        ...
        private static int FavoriteMove;
        ...
        static int RunGame(IPlayer player1, IPlayer player2)
        {
            ...
            Console.WriteLine("\nPlayer 1: {0}     \tPlayer 2 : {1}", MoveToString(p1Choice), MoveToString(p2Choice));

            int winner = CalculateWinner(p1Choice, p2Choice);

            player1.SaveResult(p1Choice, p2Choice); //mymove/nextmove
